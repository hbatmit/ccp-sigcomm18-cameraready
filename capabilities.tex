...
%\begin{outline}
%\1 Today, the congestion control problem is solved independently by each flow in the Internet.
%\1 As a result, congestion control state is distributed amongst various flows in a domain or system.
%\1 In these cases, even flows on the same system must communicate only indirectly.
%\1 CCP can easily be modified to allow for this use case.
%    \2 We implement a proof-of-concept flow aggregator in portus.
%    \2 Aggregating congestion control algorithms must implement only one additional interface, containing two functions.
%    \2 First, since the \texttt{create()} method defines new aggregates, the aggregate must define \texttt{new\_flow()} to accept a new connection into the aggregate.
%    \2 Second, algorithms must specify how to group flows into aggregates. The aggregate is given the 5-tuple of new connections, and can hash these connections into congestion aggregates.
%    \2 For example, the simplest aggregator might aggregate all the flows in a host by aggregating the source IP address, and remove slow start for these flows, instead choosing to simply set a known good congestion window.
%\1 As a proof of concept of the power of CCP's abstractions, we implement an aggregating congestion controller aimed at reducing flow completion times on a host.
%    \2 To support this, we add an additional measurement signal exposing the amount of backlogged data per connection to CCP.
%    \2 Because the aggregating CCP has a global view of demand and congestion state, it can more efficiently allocate resources to flows which need them.
%    \2 \an{srinivas's graphs}
%\1 Aggregation Evaluation
%    \2 GRAPH: show a single congestion control controlling several aggregate flows.
%    \2 this was a small change, had to add one more interface, entirely in userspace, to allow specification of which flows to aggre gate.
%\end{outline}
