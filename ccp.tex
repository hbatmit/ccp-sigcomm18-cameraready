...
Writing an algorithm in CCP involves two considerations: logic in the CCP agent and in the datapath.
This two-tier programming model involving a slow and a fast execution environment allows
algorithms to perform computations flexibly in the CCP agent, \eg compute a cubic
root or use machine learning to determine a sending rate, while
providing high performance by retaining frequently invoked logic within
the datapath.

\ma{Again, too much repetition. We just said this in the 'Supporting per-ACK ...' paragraph above.}
...
%
The implementation of \ct{onCreate()} must install a default datapath program for that flow.
%
